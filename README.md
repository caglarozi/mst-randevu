...
**Arama motoru (SEO):** arama başlığı "Yazar Paneli: Kitap Satış ve Telif Takibi | MST Yayıncılık" ve açıklaması `MST_Randevu::SEO['uygulama']`'dadır. Randevu sayfasınınki ("Kitap Yayınlatmak İstiyorum: Ücretsiz Ön Görüşme | MST Yayıncılık") `MST_Randevu::SEO['randevu']`'dadır. Rank Math / Yoast kuruluysa ve o sayfaya panelden başlık/açıklama yazılmışsa onlara dokunulmaz, boşsa bunlar kullanılır. Sayfada yapılandırılmış veri (JSON-LD) vardır: kurum, sayfa yolu, web uygulaması ve sık sorulan sorular. Google'ın zengin sonuç testiyle kontrol edilebilir: https://search.google.com/test/rich-results

**Rank Math içerik analizi:** tam sayfa şablonlarının metni düzenleyicide değil şablonda olduğu için Rank Math "içerik yok" sanıyordu. Sayfa düzenleme ekranında `assets/rank-math.js`, sayfanın sitedeki hâlini okuyup `<main>` metnini Rank Math'in `rank_math_content` filtresiyle analize ekler (randevu, Yazar Paneli, Akademi). Sayfaya ya da düzenleyiciye bir şey yazılmaz.
